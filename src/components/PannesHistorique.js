// src/pages/PannesHistorique.js - Style Minimaliste et Moderne (Proposition 2)
...
import { 
  ...
  Loader2,
  CheckCircle, 
  Wrench, // Nouvelle icône pour la réparation
  Zap,     // Nouvelle icône pour les pannes signalées
} from "lucide-react";
...
// --- Fonctions d'aide : Statuts de Panne ---
const getStatusData = (statut) => {
  // Assumons des statuts de panne courants
  switch (statut?.toLowerCase().replace(/ /g, '_')) {
    case "resolue":
      return { 
        label: "Résolue", 
    ...
    case "en_reparation":
      return { 
        label: "En Réparation", 
        color: "bg-orange-500 text-white", // Orange pour l'action
        icon: Wrench 
      };
    case "signalee":
    case "ouverte":
      return { 
        label: "Signalée", 
        color: "bg-red-600 text-white", // Rouge pour l'urgence
        icon: Zap 
      };
    default:
      return { 
...
// --- Composants Extraits (Style Minimaliste et Moderne) ---
    ...
    <nav className="flex flex-col gap-1 mt-6">
      {MENU_ITEMS.map((item) => (
        <button
          ...
          // Style net et actif: Bordure colorée pour l'élément actif
          className={`flex items-center gap-4 px-4 py-3 rounded-lg font-medium text-left text-blue-100 hover:bg-blue-700 hover:text-white transition-all duration-200 border-l-4 border-transparent active:bg-blue-700
            ${item.path === "/panneshistorique" ? 'bg-blue-700 text-white border-yellow-300' : 'hover:border-blue-300'}`}
        >
...
const DashboardHeader = ({ user, handleLogout, setMenuOpen, profileMenu, setProfileMenu }) => (
  <header className="bg-white sticky top-0 z-40 shadow-lg px-8 py-4 flex justify-between items-center border-b border-gray-200">
    <div className="flex items-center gap-4">
      ...
      <h1 className="text-3xl font-bold text-gray-800">Historique des Pannes</h1>
    </div>
    <div className="relative">
      ...
      {profileMenu && (
        <div className="absolute right-0 mt-3 w-56 bg-white shadow-xl rounded-lg border border-gray-100 overflow-hidden z-50">
...
// --- Composant Principal : PannesHistorique ---

export default function PannesHistorique() {
  ...
  // 1. Gestion de l'authentification et récupération des pannes
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data, error: pannesError } = await supabase
        .from("pannes")
        .select("*")
      ...
      if (pannesError) {
        console.error(pannesError);
        setError("Impossible de charger l'historique des pannes.");
      } else {
        setPannes(data || []);
      }
      setLoading(false);
            ...
            <nav className="flex flex-col gap-1 mt-4">
              {MENU_ITEMS.map((item) => (
                <button
                  ...
                  className={`flex items-center gap-4 px-4 py-3 rounded-lg font-medium text-left text-blue-100 hover:bg-blue-700 hover:text-white transition-all duration-200 ${item.path === "/panneshistorique" ? 'bg-blue-700 text-white' : ''}`}
                >
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg border-t-4 border-red-500">
            ...
            {loading ? (
                <div className="flex justify-center items-center p-8">
                    ...
                    <p className="text-lg text-gray-600">Chargement des pannes...</p>
                </div>
            ) : pannes.length === 0 ? (
              <p className="text-gray-500 italic p-4 text-center">Aucune panne n'a été signalée à ce jour.</p>
            ) : (
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50 border-b-2 border-gray-200">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider hidden md:table-cell">Date Signalement</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-100">
                    {pannes.map(p => {
                        const status = getStatusData(p.statut);
                        return (
                        <tr key={p.id_uuid} className="hover:bg-red-50/50 transition duration-150">
                            <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-800 truncate max-w-[150px]">{p.titre}</td>
                            <td className="px-4 py-3 text-sm text-gray-600 hidden sm:table-cell truncate max-w-xs">{p.description || "-"}</td>
                            ...
                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 hidden md:table-cell">{new Date(p.created_at).toLocaleDateString('fr-FR', { year: 'numeric', month: 'short', day: 'numeric' })}</td>
                        </tr>
                        );
                    })}
                  </tbody>
